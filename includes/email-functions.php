...
function enviar_reclamo_por_correo(
    ...
    // Definir destinatario y encabezados para correo HTML
    $para = get_option('admin_email', 'soporte@tudominio.com');
    $asunto = 'Nuevo Reclamo - ' . $tipo_reclamo;
    $headers = array('Content-Type: text/html; charset=UTF-8');

    // Si se proporcionó el correo del consumidor, agregarlo en BCC
    if (!empty($correo)) {
        $headers[] = 'Bcc: ' . $correo;
    }
